...
namespace Player
{
    ...
    public class PlayerMovement : MonoBehaviour
    {
        ...
        {
            speed = new Vector2(Input.GetAxisRaw("Horizontal") * speedXMultiply, rb.linearVelocity.y); // Global var to receive speed in X with input 

            // If to check if player has used dash or not
            if (dashTime > 0.4)
            {
                rb.linearVelocity = speed; // rb velocity receive speed
            }
